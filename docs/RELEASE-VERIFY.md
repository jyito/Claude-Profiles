...
| 12 | Look at any running card's status line | It shows a **"N leaked"** stat (amber) whenever the instance holds any leaked terminal handles — visible on the collapsed card. It **brightens** once N crosses the threshold (~50). Then open **+ Details** → **Restart to free handles** → **Confirm restart**: that instance quits and relaunches (still signed in) and the count drops. Other instances untouched. |
